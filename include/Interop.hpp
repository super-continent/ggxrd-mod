...
namespace library_private
{
    bool AreGNamesValid()
    {
        if (FName::Names() && !FName::Names()->empty() && (FName::Names()->capacity() > FName::Names()->size()))
        {
            return true;
        }
    ...
    {
        if (UObject::GObjObjects() && !UObject::GObjObjects()->empty() && (UObject::GObjObjects()->capacity() > UObject::GObjObjects()->size()))
        {
            return true;
        }
...
int select_replay(int idx)
{
    ...
    replay->vRPLCurrentFactor = idx;
    return 0;
}

int get_current_replay()
{
    auto replay = GetInstanceOf<UREDGfxMoviePlayer_MenuReplay>();
    if (!replay)
    {
...
int replay_list_length()
{
    ...
    return replay->GetReplayListNum(replay->vRPLCurrentCategory);
}
...
int set_selected_stage(int stage_id) {
    ...
    StageSelect->StageCursor = stage_id;
}
